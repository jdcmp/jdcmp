...
/**
 * Getter for double arrays.
 *
 * @param <T> Type of the object containing the double array
 */
@FunctionalInterface
public interface DoubleArrayGetter<T> extends OrderingCriterion<T>, GenericGetter<T, double[]> {
	...
	default int compare(T self, T other) {
		return DoubleArrayGetter.compare(apply(self), apply(other));
	}
